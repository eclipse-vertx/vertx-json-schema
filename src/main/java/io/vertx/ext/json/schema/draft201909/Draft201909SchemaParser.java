...
import io.vertx.ext.json.schema.draft7.ItemsValidatorFactory;
import io.vertx.ext.json.schema.draft7.*;
...
public class Draft201909SchemaParser extends BaseSchemaParser {
   ...
   */
  public static Draft201909SchemaParser create(SchemaRouter router) {
    return new Draft201909SchemaParser(router);
  }
  ...
  public static Schema parse(Vertx vertx, JsonObject schema, URI scope) {
    return new Draft201909SchemaParser(SchemaRouter.create(vertx, new SchemaRouterOptions())).parse(schema, scope);
  }
}
